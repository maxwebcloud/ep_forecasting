...
def split_data_time_series_expanding_auto_folds(
        data, target,
        n_folds=5,
        val_size_frac=0.2):
    """
    Expanding-window CV:
    Fold k trainiert von Start bis `train_end_k`
    und validiert auf das _darauffolgende_ Val-Fenster
    gleicher Länge wie `val_size_frac · train_end_k`.
    ...
    # Länge des Val-Fensters relativ zur _jeweiligen_ Train-Länge
    for k in range(n_folds):
        # proportionelles Wachstum: End-Idx des Train-Fensters
        train_end = int((k + 1) * data_len / (n_folds + 1))
        val_len   = int(train_end * val_size_frac)

        val_start = train_end
        val_end   = min(val_start + val_len, data_len)

        train_data, train_target = data[:train_end], target[:train_end]
        val_data,   val_target   = data[val_start:val_end], target[val_start:val_end]
...
def hyperparameter_tuning(Model, folds, seed, device):
    ...
    study.optimize(objective, n_trials=50, n_jobs=1, callbacks=[callback]) #n_jobs necessary for reoproducibility
...
def cross_validate_time_series(models, seeds, X, y, device , train_size=0.6, val_size=0.2, test_size=0.2, 
    ...
    # Folds  ausschliesslich aus den 77 % Train-Daten 
    ...
    #Funktionsaufruf für expanding: slidwin davior immer auskommentieren 
    folds = split_data_time_series_expanding_auto_folds(
            X_train_full, y_train_full,
            n_folds       = n_folds,
            val_size_frac = 0.2)  
