...
def save_best_hp(modelName, study):
    best_hp = study.best_trial.params
    with open(f"best_hp_all_models/best_hp_{modelName}.json", "w") as f:
        json.dump(best_hp, f)


# load best hyperparameters 
def load_best_hp(modelName):
    with open(f"best_hp_all_models/best_hp_{modelName}.json", "r") as f:
        best_hp = json.load(f)
...
def final_model_training(X_train, best_hp, Model, modelName,train_dataset, val_dataset, test_dataset, SEED):
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(final_model.state_dict(), f"saved_models/{modelName}_model_final.pth") #save best weights
...
def train_history_plot(train_loss_history, val_loss_history):
...
# load trained model  
def load_model(Model, hp, X_train, modelName):
    model_final = Model(input_size=X_train.shape[2], hp=hp)
    model_final.load_state_dict(torch.load(f"saved_models/{modelName}_model_final.pth"))
    model_final.eval()
    return model_final
...
def plot_forecast(seq_length, df_final_viz, train_predictions_actual, val_predictions_actual, test_predictions_actual, modelName):
    ...
    plt.title(f'Electricity Price Time Series Forecasting ({modelName.upper()})')
    plt.xlabel('Date')
    plt.ylabel('Price')
    plt.legend()
    plt.show()
...
def plot_residuals_with_index(y_true, y_pred, df_final_viz, seq_length, title="Prediction Residuals"):
...
def print_num_parameters(final_model):
