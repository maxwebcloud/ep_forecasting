...
def cross_validate_time_series(models, seeds, df_final, device , train_size=0.6, val_size=0.2, test_size=0.2, 
                               sequence_length=24, step_size=1, n_folds = 5, variance_ratio=0.8, single_step= True):
    
    console = Console()

    # aus Datenframe Features (vorläufig) und Target entnehmen
...
results = cross_validate_time_series([SimpleRNN], [81], df_final,torch.device("cpu"))
print(results)
"""
